...
begin
  case ACastling of
    caWH: begin y := 1; dx := -1; xto := 7; c := pcWhite; end;
    caWA: begin y := 1; dx :=  1; xto := 3; c := pcWhite; end;
    caBH: begin y := 8; dx := -1; xto := 7; c := pcBlack; end;
    caBA: begin y := 8; dx :=  1; xto := 3; c := pcBlack; end;
  end;
  x := FData.FCastling[ACastling];
  repeat
    x := x + dx;
  ...
  begin
    ...
    if (y2 = y)
    and (x2 >= Min(x, {FData.FCastling[ACastling]}xto))
    and (x2 <= Max(x, {FData.FCastling[ACastling]}xto)) then
    begin
      {$IFDEF DEBUG}WriteLn({$I %FILE%}, ' ', {$I %LINE%}, ' Roque impossible (menace ', FList[i], ')');{$ENDIF}
      result := TRUE;
    end else
      Inc(i);
  end;
